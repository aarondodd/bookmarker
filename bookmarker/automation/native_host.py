"""Native-messaging host: bridges Chrome stdio <-> the running app's loopback.

Chrome spawns this entry point every time the extension calls
``chrome.runtime.connectNative('org.aarondodd.bookmarker.sync')``. We read
``bridge.json`` to find the app's loopback port + auth token, open a TCP
connection, send a handshake, then ferry length-prefixed JSON frames between
stdin/stdout (the extension hop) and the TCP socket (the app hop) until either
side closes.

Invoked from ``main.py`` when ``--native-host`` is present in argv. The Chrome
native-messaging manifest points at a wrapper that runs the frozen executable
with that flag, so no separate bridge binary needs to ship.
"""
...
def run(handshake_file: Path, *, extension_id: str = "") -> int:
    """Entry point invoked from ``main.py`` when ``--native-host`` is present.
    ...
    if not ack or ack.get("type") != messages.HANDSHAKE_ACK or not ack.get("accepted"):
    ...
    t1 = threading.Thread(
        ...
        name="nh-ext-to-app", daemon=True,
    )
    t2 = threading.Thread(
        target=_pump_app_to_extension, args=(tcp_stream, done),
        name="nh-app-to-ext", daemon=True,
    )
    t1.start()
    t2.start()
    done.wait()
...
def _setup_stdio_binary() -> None:
    """Ensure stdin/stdout are byte streams. On Windows the default text-mode
    streams would corrupt the length prefix by translating CRLF."""
...
def _write_protocol_error(detail: str) -> None:
    _write_to_extension(messages.error("bridge_unavailable", detail))
