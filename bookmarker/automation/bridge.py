...
Why a TCP loopback hop and not just make the host be the app? Native-messaging
hosts are spawned by Chrome per ``connectNative()`` call and live only as long as
the extension keeps the connection open. Bookmarker is a long-running PyQt6 GUI
that owns the store. The native host therefore runs as a tiny bridge process that
ferries length-prefixed JSON between Chrome's stdio and the app's loopback socket.

Threading: the TCP listener + per-connection reader live in worker threads owned
by ``Bridge``. Inbound messages dispatch through ``on_message`` on the bridge
thread; the caller re-marshals onto the Qt main thread (controller.py does this
via pyqtSignal). Only one peer at a time; a second connection is dropped.

Auth: ``start()`` writes ``bridge.json {port, token, pid}``. The host reads it,
connects, and sends ``handshake{token}`` as its first frame. The token rotates
per launch (``secrets.token_hex``); a mismatched token closes the socket.

Pure Python (no Qt) so the test suite exercises it without a Qt runtime.
"""
...
# connection. The host sends it immediately on connect; anything slower is
# almost certainly a port scan or an unrelated localhost client.
...
class Bridge:
    ...
    Lifecycle::
        ...
        bridge = Bridge(handshake_file=path, on_message=cb)
    ...
    def __init__(
        ...
        *,
        on_connect: Optional[Callable[[HandshakeState], None]] = None,
    ...
    def start(self) -> None:
        """Bind a loopback port, write the handshake file, spawn the accept
        thread. Idempotent."""
    ...
    def stop(self) -> None:
        """Close sockets, remove the handshake file, join threads."""
        ...
        if peer is not None:
            try:
                ...
                pass
            peer.close()
        # accept() doesn't unblock on socket close on all platforms; poke it.
        try:
            wakeup = socket.create_connection(("127.0.0.1", self._port), timeout=0.5)
            wakeup.close()
        except OSError:
            pass
        try:
    ...
    def send(self, payload: dict[str, Any]) -> bool:
        """Push one message to the connected host. Returns False if no peer is
        connected."""
        with self._peer_lock:
            peer = self._peer_sock
        if peer is None:
            return False
        try:
            peer.sendall(encode_message(payload))
            return True
    ...
    def _handle_new_peer(self, conn: socket.socket) -> None:
        ...
        try:
            ...
            conn.close()
            return
        if not first or first.get("type") != messages.HANDSHAKE:
        ...
        if not isinstance(token, str) or not _consteq(token, self._token):
            try:
                write_message(
                    stream,
                    messages.handshake_ack(
                        accepted=False, app_version=self._app_version, detail="invalid token"
                    ),
                )
            except OSError:
                pass
        ...
        try:
            write_message(
                stream,
                messages.handshake_ack(
                    accepted=True, app_version=self._app_version, detail="ok"
                ),
            )
        except OSError as exc:
        ...
        if self._on_connect is not None:
            ...
            except Exception:  # noqa: BLE001 -- defensive; don't kill the bridge
    ...
    def _write_handshake_file(self) -> None:
        payload = {"port": self._port, "token": self._token, "pid": os.getpid()}
