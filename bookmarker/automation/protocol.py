...
Chrome native messaging uses a length-prefixed JSON protocol over the host's
stdio: each message is a 4-byte little-endian unsigned int followed by that many
UTF-8 bytes of JSON. We reuse the exact same framing on the TCP loopback hop
between the native host and the running app, so the host is a near-trivial byte
copier.
...
# Native messaging caps payloads at 1 MB inbound. A full bookmark tree fits
# comfortably; keep the ceiling explicit so a malformed length field can't make
# us try to allocate gigabytes.
MAX_MESSAGE_BYTES = 4 * 1024 * 1024


class ProtocolError(Exception):
    """Raised on framing violations: short reads, oversize messages, or
    non-UTF-8 / non-JSON payloads."""
...
def read_message(stream: BinaryIO) -> Optional[dict[str, Any]]:
    """Read one framed message. Returns None on clean EOF (peer closed between
    messages). Raises ProtocolError on framing violations."""
...
def _read_exact(stream: BinaryIO, n: int) -> Optional[bytes]:
    """Read exactly ``n`` bytes. Returns None if the stream is at EOF before any
    data has been read; raises ProtocolError on partial read (peer closed
    mid-message)."""
    chunks: list[bytes] = []
    remaining = n
    while remaining > 0:
